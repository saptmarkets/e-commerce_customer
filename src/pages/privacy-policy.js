...
const PrivacyPolicy = () => {
  ...
  const sections = useMemo(() => {
    return words
      .map((w, i) => {
        const title = getEn(pp[`section_${w}_title`]);
        const body = getEn(pp[`section_${w}_body`]);
  ...
  const rights = useMemo(() => {
    ...
    return labels
      .map((w) => {
        const name = getEn(pp[`right_${w}_name`]);
        const desc = getEn(pp[`right_${w}_desc`]);
        const emoji = getEn(pp[`right_${w}_emoji`]);
    ...
    <Layout title="Privacy Policy" description="SAPT Markets data protection policy">
      ...
      <div className="bg-[#F5F1FA] w-full px-4 pb-20 pt-12">
        <SectionBox className="max-w-screen-2xl mx-auto">
          ...
          {/* rich desc */}
          {getEn(pp.description) && (
            <div className="prose max-w-none mt-12" dangerouslySetInnerHTML={{ __html: getEn(pp.description) }} />
          )}
        </SectionBox>
