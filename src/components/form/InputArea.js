...
const InputArea = ({
  ...
  // Determine if this is an email field that needs validation
  const isEmailField = type === 'email';
  
  // Apply email pattern validation for email fields
  const validationRules = {
    required: required ? `${label} is required!` : false,
    pattern: isEmailField 
      ? { value: emailPattern, message: "Please enter a valid email address" }
      : pattern 
